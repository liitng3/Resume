  ...
  onLoad: function (options) {
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        ctx.moveTo(0, 80)
        ctx.lineTo(300,80)
        ctx.setStrokeStyle('#7f3d3c')
        ctx.setLineWidth(0.5)
        ctx.closePath()
        ctx.setFontSize(8)
        ctx.setFillStyle('black')
        ctx.fillText('姓名：'+that.data.name,95,25)
        ctx.fillText('年龄：'+that.data.age,95,35)
        ctx.fillText('性别：'+that.data.sex,95,45)
        ctx.fillText('民族：'+that.data.nation,95,55)
        ctx.fillText('婚姻状况：'+that.data.marry,165,25)
        ctx.fillText('政治面貌：'+that.data.face,165,35)
        ctx.fillText('手机号：'+that.data.tel,165,45)
        ctx.fillText('邮箱：'+that.data.email,165,55)
        ctx.stroke()
        ...
        ctx.fillRect(24, 5, 50, 66.875)

        wx.getImageInfo({
          src: that.data.imgUrl,
          success(res){
            ctx.drawImage(res.path,25,6,48,64.875)
            ctx.draw()
          }
        })
      },
